...
[.text-justify]
L'application est disponible à l'exécution et sans prérequis autre qu'une JRE et que le script `script.py`, sous forme de fichier JAR. Elle peut être régénérée à partir du code source et d'un IDE Java, mais ce dernier devra disposer d'une JRE Java versionnée en 1.8 ainsi que toutes les librairies nécessaires. Les lignes de codes suivantes doivent d'abord être modifiées pour chaque classe :

* `DialogueController.java` ligne 34 :
[source, java]
loader.setLocation(getClass().getResource("../view/mainView.fxml"));

* `Config.java` ligne 63 :
[source, java]
...
[.text-justify]
Le poste sur lequel souhaite être lancée l'application avec le fichier JAR exécutable devra préalablement être paramétré avec une JRE versionné en 1.8.

Le script Python doit être placé dans le même répertoire que le fichier JAR et être lancé *avant* ce dernier. Pour cela, les commandes suivantes peuvent être utilisées dans un terminal :
...
(essayez l'autre si l'une ne marche pas), en vous plaçant au préalable dans le répertoire englobant le script et le fichier JAR de l'application.

[NOTE]
====
[.text-justify]
Pour toute information concernant l'installation d'une JRE 1.8, se à la *documentation utilisateur*. Et pour plus d'informations à propos du script `script.py`, se référer à la documentation de ce dernier.
====
