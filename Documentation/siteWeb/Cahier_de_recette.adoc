...
^|2 ^|L'utilisateur clique sur la sous-catégorie "Samsung" de la catégorie "Smartphone" ^|Tous les Smartphones Samsung sont affichés  ^|OK
^|3 ^|L'utilisateur écrit "Ordinateur" dans la barre de recherche ^|Tous les produits "Ordinateur" sont affichés  ^|OK
^|4 ^|L'utilisateur clique sur un produit affiché ^|Il est redirigé vers la page de consultation de ce produit  ^|OK
...
^|3 ^|Remplir tous les champs correctement ^|Affichage d'un message indiquant la réussite de l'ajout et insertion du produit dans la base de données ^|OK
...
^|2 ^|Essayer d'accéder à la page en étant connecté en tant qu'administrateur ^|Accès à la page ^|OK
^|3 ^|Remplir des champs ne correspondant pas aux données à mettre dans la BD ^|Affichage d'une erreur ^|OK
^|3 ^|Remplir tous les champs correctement ^|Affichage d'un message indiquant la réussite de l'ajout et insertion de la catégorie dans la base de données ^|OK
...
>|Objectif 3+|Les administrateurs du serveur peuvent supprimer une catégorie de la base de données, les produits qui y sont affectés seront désactivés
